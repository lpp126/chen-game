...
/** 与棋盘容器 top 一致（7.5rem @ 32px 根字号） */
const BOARD_TOP = 240;
const TRAY_TOP = BOARD_TOP + BOARD_SIZE + 14;
...
const pieceCenter = (piece: PieceData) => ({
  ...
  y: BOARD_TOP + piece.row * CELL + CELL / 2
});
...
/** 旧吸附玩法完整备份，勿删除。主文件见 Level5PuzzleGame.tsx */
export const Level5PuzzleGameLegacy: React.FC = () => {
  const {
    status,
    currentLevelId,
    gameplayPaused,
  ...
  useEffect(() => {
    ...
    const onMove = (event: PointerEvent) => {
      ...
      const dist = Math.hypot(home.x - centerX, home.y - centerY);
      const snapped = dist < SNAP_DISTANCE;
      setDrag((cur) => (cur ? { ...cur, x: snapped ? home.x : centerX, y: snapped ? home.y : centerY, snapped } : cur));
    };
  ...
  return (
    ...
    >
      ...
      <div className="absolute left-0 right-0 flex justify-center" style={{ top: BOARD_TOP }}>
          ...
          {!showFinalReveal && drag?.snapped && (
            <div className="absolute inset-0 pointer-events-none">
              <div
                ...
                style={{ left: drag.x - BOARD_X - 20, top: drag.y - BOARD_TOP - 20 }}
              />
            </div>
          )}
        </div>
      ...
      <div
        className={`absolute left-0 right-0 bottom-20 px-4 ${showFinalReveal ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
        style={{ top: TRAY_TOP }}
      >
        <div className="h-full rounded-[18px] border border-white/40 bg-white/30 backdrop-blur-[2px] overflow-y-auto px-2 py-2">
            ...
            {pieces
              ...
              .map((piece) => (
                <button
                  ...
                  className={`relative p-0.5 rounded-lg bg-white/50 border border-white/45 shadow-[0_2px_6px_rgba(0,0,0,0.06)] ${shakeId === piece.id ? 'animate-[pulse_0.2s_ease-in-out]' : ''}`}
                >
                  <svg viewBox={`0 0 ${PIECE_SIZE} ${PIECE_SIZE}`} className="w-full h-auto">
                    ...
                    />
                    <path d={piecePath(piece)} fill="none" stroke="rgba(255,255,255,0.72)" strokeWidth={0.75} />
                  </svg>
                </button>
              ))}
          </div>
